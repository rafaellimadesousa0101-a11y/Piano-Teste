...
let toolbarContent = fs.readFileSync('src/components/MobileTouchToolbar.tsx', 'utf8');

const search = `    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] p-2 pb-safe flex flex-col z-50">
      {/* Primary Tools Row */}
...
const replace = `    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] p-2 pb-safe flex flex-col gap-2 z-50">
      {/* Action Row - only visible when a note is selected */}
      {hasSelectedNote && (
...
toolbarContent = toolbarContent.replace(search, replace);
fs.writeFileSync('src/components/MobileTouchToolbar.tsx', toolbarContent);
